...
window.Vaadin.Flow['_vaadintheme_musicfinder_globalCss'] = window.Vaadin.Flow['_vaadintheme_musicfinder_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_musicfinder_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_musicfinder_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_musicfinder_globalCss'].push(target);
  }
  if (!document['_vaadintheme_musicfinder_componentCss']) {
    
    document['_vaadintheme_musicfinder_componentCss'] = true;
  }
